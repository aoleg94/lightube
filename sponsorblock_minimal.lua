...
local options = {
    ...
    --API = "http://127.0.0.1:5000/api/ytdl/https%3A%2F%2Fsponsor.ajay.app%2Fapi%2FskipSegments%3Fcategories%3D%5B%22sponsor%22%2C%22intro%22%2C%22outro%22%2C%22interaction%22%2C%22selfpromo%22%5D%26videoID%3D",
    API = 'https://sponsor.ajay.app/api/skipSegments?categories=["sponsor","intro","outro","interaction","selfpromo"]&videoID=',

    -- Categories to fetch and skip
    categories = '"sponsor","intro","outro","interaction","selfpromo"'
}

function getranges(youtube_id)
	local args = {
		"youtube-dl",
		"--invalid-option",
		"--",
		options.API .. youtube_id
	}
	--[["curl",
		"-s",
		"-d",
...
function file_loaded()
	...
	local youtube_id = youtube_id1 or youtube_id2 or youtube_id3 or youtube_id4
	if not youtube_id or string.len(youtube_id) < 11 then return end
	youtube_id = string.sub(youtube_id, 1, 11)

	getranges(youtube_id)
	if ranges then
		ON = true
